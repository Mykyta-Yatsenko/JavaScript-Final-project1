...
const searchClick = () => {
    ...
    if (!Data[sText.value]) {
        if (!Data.countries.find(item => item.name === sText.value)) {
            sText.value = 'Input value exactly.';
        }else {
...
function cardMake(imageUrl, title, content) {
    const card_content = document.getElementById('card-content');
    const card = document.createElement('div');
    const str = `<img src="../img/${imageUrl}"><h2>${title}</h2><p>${content}</p> <input type = 'button' value = 'VISIT'>`;
    card.innerHTML = str;
    card.className = 'card';
